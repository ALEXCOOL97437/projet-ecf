...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if ($recupManager->rowCount() > 0) {
        ...
        $managerInfos = $recupManager->fetch();
        $pseudo = $managerInfos['pseudo'];
        $mdp_exist = $managerInfos['mdp'];  //Récupérer le mot de passe existant depuis la base de données
        ...
        if (isset($_POST['valider'])) {
            ...
            $updateManager = $bdd->prepare('UPDATE manager SET pseudo = ?, mdp = ? WHERE id = ?');
            $updateManager->execute(array($pseudo_saisi, $mdp_saisie, $getid));
...
<head>
    ...
    <title>Modifier les manageurs</title>
